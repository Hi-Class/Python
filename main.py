...
# from src.speaker import get_parameter
import src.speaker.get_meal_schedule as get_meal_schedule
import src.speaker.get_parameter as get_parameter
import src.speaker.get_youtube_mp3 as get_youtube_mp3
import src.speaker.env as env
...
class SampleAssistant(object):
    ...
    def __init__(self, language_code, device_model_id, device_id,
        ...
        self.device_model_id = env.DEVICE_MODEL_ID
        ...
        self.conversation_stream.volume_percentage = 100 # Add
    ...
    def assist(self):
        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(),
            ...
            if resp.speech_results:
                logging.info(' '.join(r.transcript for r in resp.speech_results))
                logging.info('Transcript of user request: "%s".',
                             ' '.join(r.transcript
                                      for r in resp.speech_results))
                result_text=resp.speech_results[0].transcript
            ...
            if len(resp.audio_out.audio_data) > 0:
                ...
                if ('하이' in result_text) and ('클래스' in result_text):
                    ...
                    if '급식' in result_text:
                        ...
                        meal=get_meal_schedule.meal(date, meal_time)
                        try:
                            os.system('gtts-cli '+ "'" + meal + "'" +' --lang ko | mpg123 -')
                        except:
                            pass
                        logging.info('meal_data: ' + meal)
                        break
                    elif '시간표' in result_text:
                        date, grade, number=get_parameter.schedule(result_text)
                        schedule=get_meal_schedule.schedule(date, grade, number)
                        try:
                            os.system('gtts-cli '+ "'" + schedule + "'" +' --lang ko | mpg123 -')
                        except:
                            pass
